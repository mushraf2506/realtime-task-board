/**
 * Client-side fractional indexing — mirrors the backend orderingService.
 * Used for optimistic UI: compute position before server confirms.
 */

export const MIN_GAP = 1e-8;
export const INITIAL_STEP = 1000;

export function midpoint(a: number, b: number): number {
  return (a + b) / 2;
}

export function before(firstPosition: number): number {
  return firstPosition / 2;
}

export function after(lastPosition: number): number {
  return lastPosition + INITIAL_STEP;
}

export function needsRebalance(a: number, b: number): boolean {
  return Math.abs(b - a) < MIN_GAP;
}

export function computeInsertPosition(
  tasks: Array<{ position: number }>,
  insertIndex: number
): number {
  if (tasks.length === 0) return INITIAL_STEP;
  if (insertIndex <= 0) return before(tasks[0].position);
  if (insertIndex >= tasks.length) return after(tasks[tasks.length - 1].position);
  return midpoint(tasks[insertIndex - 1].position, tasks[insertIndex].position);
}
